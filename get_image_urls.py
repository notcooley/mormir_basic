...
creatures = {}
#I dont want these stinkers
no_nos = = [
    "40K",          # Warhammer 40,000
    "WHO",          # Doctor Who
    "JUR",          # Jurassic World Collection
    "BOT",          # Transformers (The Brothers' War bonus)
    "ACR",          # Assassin's Creed
    "PIP",          # Fallout
    "FIN",          # Final Fantasy
    "TMT",          # Teenage Mutant Ninja Turtles
    "ATLA",         # Avatar: The Last Airbender
    "LTR_HOLIDAY"   # The Lord of the Rings: Holiday Edition (only LOTR inclusion)
]
        ...
        try:
            for key, value in item.items():
                if "Creature" in value[0]["type"]:
                    set = values[0].get("printings", [None])[0]
                    if value[0]["legalities"] and "A-" not in value[0]["name"] and not values[0].get("isFunny", False) and set not in no_nos:
                        creatures[key] = value       
